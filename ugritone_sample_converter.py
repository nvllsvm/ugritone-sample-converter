...
DECRYPT_SAMPLE_KEY = base64.b64decode(b"GgQFF0D1LQMrVT8tAgOmrzV7k2U=")
DECRYPT_DEMO_SAMPLE_KEY = base64.b64decode(b"IiQpQAgEBwcXQQXqBQEOKwIXNgE=")


class Blowfish_ECB_LittleEndian:
    """
    Derived from https://github.com/jashandeep-sohi/python-blowfish (GPLv3).
    Embedded to avoid an external dependencies.
    """
    ...
    _U4_2_STRUCT = struct.Struct("<2I")
    _U4_1_STRUCT = struct.Struct(">I")
    _U1_4_STRUCT = struct.Struct("=4B")

    def __init__(self, key):
        cyclic_key_iter = itertools.cycle(iter(key))
        cyclic_key_u4_iter = (
            x
            for (x,) in map(
                self._U4_1_STRUCT.unpack,
                map(
                    bytes,
                    zip(
                        cyclic_key_iter,
        ...
        S1, S2, S3, S4 = S = [[x for x in box] for box in self.PI_S_BOXES]

        L = 0x00000000
        R = 0x00000000

        for i in range(len(P)):
            P[i] = L, R = self._encrypt(L, R, P, S1, S2, S3, S4)
        self.P = P = tuple(P)

        for box in S:
            for i in range(0, 256, 2):
                L, R = self._encrypt(L, R, P, S1, S2, S3, S4)
                box[i] = L
                box[i + 1] = R
        self.S = tuple(tuple(box) for box in S)

    def _encrypt(self, L, R, P, S1, S2, S3, S4):
        for p1, p2 in P[:-1]:
            L ^= p1
            a, b, c, d = self._U1_4_STRUCT.unpack(self._U4_1_STRUCT.pack(L))
            R ^= (S1[a] + S2[b] ^ S3[c]) + S4[d] & 0xFFFFFFFF
            R ^= p2
            a, b, c, d = self._U1_4_STRUCT.unpack(self._U4_1_STRUCT.pack(R))
            L ^= (S1[a] + S2[b] ^ S3[c]) + S4[d] & 0xFFFFFFFF
        p_penultimate, p_last = P[-1]
        return R ^ p_last, L ^ p_penultimate

    def _decrypt(self, L, R, P, S1, S2, S3, S4):
        for p2, p1 in P[:0:-1]:
            L ^= p1
            a, b, c, d = self._U1_4_STRUCT.unpack(self._U4_1_STRUCT.pack(L))
            R ^= (S1[a] + S2[b] ^ S3[c]) + S4[d] & 0xFFFFFFFF
    ...
    def decrypt(self, data):
        ...
        LR_iter = self._U4_2_STRUCT.iter_unpack(data)

        data = b"".join(
            self._U4_2_STRUCT.pack(
                *self._decrypt(cipher_L, cipher_R, P, S1, S2, S3, S4)
            )
        ...
        padding_size = data[-1]
        if not (0 < padding_size <= 8 <= len(data)):
...
@functools.cache
def get_cipher(key):
    return Blowfish_ECB_LittleEndian(key)
...
def _process_path(path, target, key, delete=False):
    encrypted = path.read_bytes()
    decrypted = get_cipher(key).decrypt(encrypted)
    safe_write_bytes(target, decrypted)
    if delete:
        path.unlink()
        ...
        for path in all_files(args.path):
            match path.suffix.lower():
                case ".ugrisample":
                    ...
                    suffix = ".flac"
                case ".ugridemo":
                    key = DECRYPT_DEMO_SAMPLE_KEY
                    suffix = ".ogg"
                case _:
                    continue
            target = path.parent / f"{path.name}{suffix}"
            if target.exists():
                continue
            future = executor.submit(
                _process_path, path, target, key, delete=args.rm)
            future_to_paths[future] = path, target

        errors = []
        num_printed = 0
        for future in concurrent.futures.as_completed(future_to_paths):
            path, target = future_to_paths[future]
            try:
                future.result()
